...
## Imports
```ts
import { stitch, defineStitch, preset, drift, bearer, apiKey, basic, cookieSession, env, keychain } from '../src';
...
// B) factory
const s = defineStitch(base, authStrategy); s({ path: '/x' });
// C) builder
...
server.calls('/x'); server.callCount('/x'); server.reset(); await server.close();
```
