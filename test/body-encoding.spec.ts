...
process.env.STITCH_TRACE_FILE = join(tmpdir(), `stitch-encoding-${process.pid}.jsonl`);
...
describe('Body encoding', () => {
    ...
    test('multipart -> multipart/form-data with a named file part + text fields', async () => {
        ...
        const upload = stitch({ method: 'POST', baseUrl: server.url, path: '/upload', bodyType: 'multipart' });

        const bytes = new Uint8Array([1, 2, 3, 4]);
        await upload({
            body: { category: 'movies', file: { value: bytes, filename: 'a.bin', type: 'application/octet-stream' } },
        });
    ...
    test('json remains the default', async () => {
        ...
        const j = stitch({ method: 'POST', baseUrl: server.url, path: '/json' });
...
describe('cookieSession content-aware refresh (soft wall)', () => {
    test('re-logs-in when a 200 response is actually a login page', async () => {
        ...
        server.route('POST', '/auth', { setCookie: { name: 'SID', value: 'OK' }, body: 'ok' });
        // call #0: a 200 login page (stale session); call #1: the real data.
        server.route('GET', '/data', { statuses: [200, 200], body: ['<html>please log in</html>', { items: [1, 2] }] });

        const login = stitch({ method: 'POST', baseUrl: server.url, path: '/auth', bodyType: 'form' });
        const data = stitch({
            baseUrl: server.url,
            path: '/data',
            auth: cookieSession({
                ...
                loginInput: () => ({ body: { u: env('SOFT_USER')(), p: env('SOFT_PASS')() } }),
            }),
        });
