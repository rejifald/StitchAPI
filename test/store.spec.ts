...
process.env.STITCH_TRACE_FILE = join(tmpdir(), `stitch-store-${process.pid}.jsonl`);
...
const loginInput = () => ({ body: { u: env('ST_USER')(), p: env('ST_PASS')() } });

async function wasThrottled(s: Stitch): Promise<number> {
    let throttled = 0;
    for await (const ev of s.stream()) {
        if (ev.type === 'progress' && ev.phase === 'throttled' && (ev.waitedMs ?? 0) > 0) throttled = 1;
    }
    return throttled;
}
...
describe('Pluggable store — sessions', () => {
    test('a SHARED store makes two stitches share one login', async () => {
        ...
        server.route('POST', '/login', { setCookie: { name: 'SID', value: 'OK' }, body: { ok: true } });
        server.route('GET', '/a', { requireCookie: { name: 'SID' }, body: { r: 'a' } });
        server.route('GET', '/b', { requireCookie: { name: 'SID' }, body: { r: 'b' } });
        const login = stitch({ method: 'POST', baseUrl: server.url, path: '/login', bodyType: 'form' });
        const store = memoryStore();

        const a = stitch({ baseUrl: server.url, path: '/a', store, auth: cookieSession({ login, cookie: 'SID', key: 'svc', loginInput }) });
        const b = stitch({ baseUrl: server.url, path: '/b', store, auth: cookieSession({ login, cookie: 'SID', key: 'svc', loginInput }) });
    ...
    test('default (separate) stores → each stitch logs in independently', async () => {
        ...
        const a = stitch({ baseUrl: server.url, path: '/a', auth: cookieSession({ login, cookie: 'SID', key: 'svc', loginInput }) });
        const b = stitch({ baseUrl: server.url, path: '/b', auth: cookieSession({ login, cookie: 'SID', key: 'svc', loginInput }) });
...
describe('Pluggable store — throttle', () => {
    test('a SHARED store shares the rate budget across two stitches (one of two concurrent calls waits)', async () => {
        ...
        const store = memoryStore();
        const s1 = stitch({ baseUrl: server.url, path: '/x', store, throttle: { rate: '1/s', scope: 'host' } });
        const s2 = stitch({ baseUrl: server.url, path: '/x', store, throttle: { rate: '1/s', scope: 'host' } });

        const [w1, w2] = await Promise.all([wasThrottled(s1), wasThrottled(s2)]);
    ...
    test('default (separate) stores do NOT share the rate budget', async () => {
        ...
        const s1 = stitch({ baseUrl: server.url, path: '/y', throttle: { rate: '1/s', scope: 'host' } });
        const s2 = stitch({ baseUrl: server.url, path: '/y', throttle: { rate: '1/s', scope: 'host' } });
