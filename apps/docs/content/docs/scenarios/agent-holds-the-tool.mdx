...
description: 'Exposing a vendor API to an LLM over MCP. The credential boundary held under 30 payload scans — the argument boundary is yours, and an input slot with no schema is a full passthrough.'
...
that as [#648](https://github.com/rejifald/StitchAPI/issues/648);
[#663](https://github.com/rejifald/StitchAPI/pull/663) fixed it. A declared slot now sends **what
...
- **`only`** — a `Proxy` apply-trap that **rebuilds** the input from an explicit key list before
  the engine sees it. Since [#663](https://github.com/rejifald/StitchAPI/pull/663) a declared
  schema filters its own slot, so `only` is no longer forced — it earns its place covering the
  slots a stitch leaves undeclared, in one line, and as defence in depth on the rest.
