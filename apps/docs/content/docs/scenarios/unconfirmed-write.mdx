...
3. **Timeout disambiguation.** A dropped request (**0 charges**) and a lost response (**1
   ...
   `attempts: 1`, `timed out after 5000ms`, `body: undefined` — and the same live `TimeoutError`
   on `error.cause`. The class stays unexported, but the instance rides `cause`, so "was this a
   timeout?" is the structural check `err.cause?.constructor.name === 'TimeoutError'`, not a
   message match. What it cannot tell you is whether the money moved.
