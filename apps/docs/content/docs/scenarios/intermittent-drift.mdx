...
arrives as `"12345"` produces `error | invalid | transaction_id | Invalid input: expected
number, received string` and the **call fails** with `data: null`. Even `z.coerce.number()` on
`"abc"` fails — Zod rejects `NaN`. StitchAPI does not manufacture a $0 charge on its own.
...
3. **`level` is keyed by mechanism, not by change class.** It takes
...
5. **A soft finding cannot be promoted to fatal through the type.** `error` isn't in
   the soft half of `DriftLevel` — `Exclude<DriftLevel, 'error'>` (a cast past it does work at
   runtime).
