...
There is also **no inbound-signature primitive anywhere** in the packages. Enumerating all 89
runtime exports — one level into namespace exports, so nothing hides behind one — **none** match
`/verif|hmac|…/`; the only near-misses are the BYO-plugin conformance kit. The
...
one-line change and `conformance.store` proves a BYO one conforms (measured: all 11 rules, 0
violations).
