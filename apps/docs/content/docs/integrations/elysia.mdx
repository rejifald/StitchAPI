...
Set `onError: false` to register none and wire your own with `stitchError.handler()`
(an `.onError`-compatible mapper) or `stitchError.map(err, options)` (a one-off
`StitchError` → `Response`); `stitchError.is(err)` narrows an unknown error first.

`stitchError` is the same namespace every `@stitchapi` host adapter exports for this
one concept. The **option** keeps Elysia's own word (`onError`) while the **export**
