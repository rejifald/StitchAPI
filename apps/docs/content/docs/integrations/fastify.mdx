...
Pass `seam` a config fragment and the plugin **builds and owns** the seam — closing
it on the Fastify `onClose` hook. Pass `seam` a prebuilt seam instead and the plugin
**borrows** it: the app owns its lifecycle and the plugin never closes it. The one
field carries both; the plugin tells them apart at runtime with `isSeam()`.
...
`stitchError.handler(options)`; `stitchError.is(err)` narrows an unknown error to a
`StitchError` first. The **option** keeps Fastify's own word (`errorHandler`, after
`setErrorHandler`) while the **export** is the `stitchError` namespace every
`@stitchapi` host adapter shares.
