...
from an injected factory. `forFeature({ stitches, seamConfig })` registers
injectable stitches and, optionally, a per-upstream seam built over the shared
store and trace — omit `seamConfig` to attach the stitches to the default one.
