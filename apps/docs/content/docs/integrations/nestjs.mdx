...
trace via `seam.config` — omit `seam` to attach the stitches to the default one.
`seam.token` exposes the feature seam under a caller-chosen DI token, for
multi-tenant `.as(principal)`.
