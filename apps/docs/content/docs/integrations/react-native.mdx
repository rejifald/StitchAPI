...
— while non-streaming requests delegate to core's buffered
[`xhrAdapter`](/docs/guides/transport/xhr-adapter). The `adapter` field it plugs
into is the same [transport seam](/docs/guides/transport/adapters) every stitch
has, so nothing above it changes.
...
It is proven against the same `conformance.store` kit every
