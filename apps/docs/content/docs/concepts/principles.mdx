...
gone when the process exits; nothing is persisted or shared across workers
unless you ask for it. Persistence and sharing are a single opt-in: hand a
stitch a shared `store` and the same throttle becomes a distributed rate limit
and the same session survives restarts and is shared across processes.
Distributed limiting and shared sessions are a config choice you reach for,
never a default you inherit — which is what keeps a stitch safe to import and
call anywhere. It is progressive disclosure applied to durability: ephemeral
and isolated until you opt into more.
...
-   **No side effects by default** is the [in-memory store](/docs/guides/state/pluggable-store)
    that backs every stitch until you swap it — opt into a shared store and the
    same throttle turns [distributed](/docs/guides/state/distributed-throttle)
    and the same [session is shared](/docs/guides/state/shared-sessions) across
    workers, both off by default.
