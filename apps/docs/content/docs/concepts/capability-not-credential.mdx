...
`credential.file('NAME')` don't read anything when you declare the stitch — they return
...
`credential.file()` are the lazy lookup that keeps secrets out of the declaration; the
