...
This front door runs stitches you name; it is not a webhook endpoint, and
StitchAPI does not receive webhooks at all — that is your server's job.
[Receiving a signed webhook](/docs/scenarios/webhook-receipt) draws exactly where
the line falls and what the library still owns on the far side of it.
