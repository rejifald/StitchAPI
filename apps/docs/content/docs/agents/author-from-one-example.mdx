...
- **A request body → `wire.body` + an example.** A JSON `-d` payload becomes
  `wire: { body: 'json' }`, a form one (`k=v&…` or `--data-urlencode`) becomes
  `wire: { body: 'form' }`, and a body implies `POST` unless `-G` folds the data into
  the query.
