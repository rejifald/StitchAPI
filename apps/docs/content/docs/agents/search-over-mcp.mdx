...
description: 'Connect an agent to the docs MCP — hosted at stitchapi.dev, or run offline from the local @stitchapi/docs-mcp package — so search_docs finds the relevant sections and get_doc reads a full page, without loading the whole corpus.'
...
every turn. Point it at the server we host, or [run the same two tools
offline](#run-it-offline) from a local package.
...
Everything above points at the server we host. When you'd rather nothing leave
your machine, the same two tools ship as a local package,
[`@stitchapi/docs-mcp`](https://www.npmjs.com/package/@stitchapi/docs-mcp): the
docs corpus and its search index are bundled inside it, so `search_docs` and
`get_doc` run over stdio against local files. Point your agent at the command
instead of a URL:
...
Same schemas and the same responses as the hosted server — only the transport
(stdio) and the data source (a bundled snapshot) change. Reach for it when:

-   **Privacy matters.** No query and no doc content is transmitted anywhere —
    not even the shape of what you are asking. The one exception is a one-time
    download of the embedding model on first use; every call after that is fully
    offline.
-   **Egress is locked down.** The docs are bundled at publish time, so there is
    no bespoke endpoint to allowlist — only the ordinary `npm`/`npx` resolution
    any dependency needs.
-   **You want the docs pinned.** `@stitchapi/docs-mcp@X` is a reproducible
    snapshot, rebuilt from the docs at that release, so you know exactly which
    docs a version carries. The trade against the hosted server is freshness: the
    bundle only updates when a new version ships.
