...
The credential boundary is not the only one, though: the agent is choosing those
argument values. An input slot you leave unconstrained is a passthrough the model
fills in — [the agent picks the arguments](/docs/scenarios/agent-holds-the-tool)
tests both boundaries and shows which one is yours to close.
