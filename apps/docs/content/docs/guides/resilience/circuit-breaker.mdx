...
`failures` is the count of consecutive failures that trips the breaker open
(default `5`), and `cooldown` is the fast-fail window after opening, before a
half-open trial is allowed (default `'30s'`). Both are optional, so
`circuit: { key: 'orders' }` builds a working breaker on the house defaults —
declaring `circuit` at all is the decision a reviewer needs to see; the
thresholds are tuning.
...
[one customer's revoked token, everyone's outage](/docs/scenarios/multi-tenant-blast-radius)
...
second vendor, [failing over to the backup provider](/docs/scenarios/provider-failover)
