...
so a long enough `total` is what lets a slow attempt be retried at all. Neither
scope spans separate calls, so a submit-then-poll job needs its deadline built a
level up — [submit, poll, download](/docs/scenarios/async-job-polling) works
through the three-call version and what each choice costs.
