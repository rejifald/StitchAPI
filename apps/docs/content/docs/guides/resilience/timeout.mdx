...
`total` bounds the entire call across every retry; `each` bounds a single attempt
on its own. Both accept either a number of milliseconds (`3000`) or a duration
string (`'3s'`), so `total: '10s'` and `total: 10000` are equivalent.
...
With [retry](/docs/guides/resilience/retry), the two timeouts compose: every try
is gated by `each`, while `total` caps the sum of all tries plus their backoff
waits, so a long enough `total` is what lets a slow attempt be retried at all.

<Callout type="warn">
    **Anti-pattern:** with retry enabled, don't rely on `each` alone to bound
    how long the caller waits; it caps a single try, not the call, so several
