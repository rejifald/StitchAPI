...
`on` is the `StatusMatch` treated as a rate-limit signal — a status, a list, or a
predicate — defaulting to `[429]` (`on: 429` ≡ `on: [429]`). Widen it when an API
signals back-pressure with another status (for example
`{ delegate: true, on: [429, 503] }`); a status _not_ matched by `on` is left to
