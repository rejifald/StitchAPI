...
skips the side effect, and safe retries don't double-charge. A timeout is the
hard version of this — it tells you nothing about whether the server applied the
write, and
[the charge you can't confirm](/docs/scenarios/unconfirmed-write) measures what
the default key does to a re-driven job. It isn't only for
