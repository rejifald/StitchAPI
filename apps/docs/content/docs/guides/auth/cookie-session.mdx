...
`refresh` re-runs the login on a status code (default `[401]`, via a bare
scalar/array or `{ on }`); pass `{ when }` to re-run it on a content predicate
instead, for soft 200 walls where a login page is served with a `200`. Give two
stitches the same `key` plus a shared `store` to share one session. See
...
or retries (that stays governed by `refresh.on`/`refresh.when`). Your external loop
reads the durable state and decides when to call the stitch again.

<Callout type="warn">
    **Anti-pattern:** don't lean on `refresh` status codes alone to catch an
