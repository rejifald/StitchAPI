...
Point `login` at the stitch that authenticates, and pass `credentialsOf` to supply
...
- `onFailure(info)` runs when an attempt captured no cookie, with a
...
const me = stitch({
    ...
    auth: cookieSession({
        ...
        onFailure: async (info: AuthFailureResult) => {
