...
variable or `credential.file('API_TOKEN')` for `~/.stitch/secrets.json`; both return a thunk
...
## Optional credentials with `env.optional`
...
`env.optional('NAME')` and hand it to `bearer`. It resolves the variable's value,
or _absent_ when the variable is unset. Unlike `bearer(env('NAME'))`, which
**throws** on a missing variable, `bearer(env.optional('NAME'))` attaches the
...
const repos = stitch({
    ...
    auth: bearer(env.optional('GITHUB_TOKEN')),
});
```
...
<Callout type="warn">
    **Optional, not required.** If the variable is unset,
    `bearer(env.optional(…))` sends **no** credential rather than failing — the
