...
The token is cached and auto-refreshed via the `refresh` envelope: `refresh: {
skew }` (default `30_000`) refreshes it that many milliseconds before expiry,
so it is never used mid-flight, and `refresh: { on }` (default `[401]`) lists
the statuses that mean the token was rejected and force a fresh fetch plus
retry. A bare `refresh: <status>` is shorthand for `{ on: <status> }`.
