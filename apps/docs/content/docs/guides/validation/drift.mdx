...
`level` controls how soft drift is leveled or filtered. Three shapes:
...
const a = stitch({
    ...
    output: drift(z.object({ id: z.number() }), {
        level: 'warn', // only coerced findings surface; undeclared (info) and defaulted (verbose) are dropped
    }),
});
...
const b = stitch({
    ...
    path: '/b',
    output: drift(z.object({ id: z.number() }), {
        level: ['info', 'warn'], // drop verbose (defaulted), surface the rest
    }),
});
...
const c = stitch({
    ...
    path: '/c',
    output: drift(z.object({ id: z.number() }), {
        level: { coerced: 'info', defaulted: 'info' },
    }),
});
```

Omit `level` to surface all soft drift at its default level.
