...
`onResponse` fires on **any** status, which makes it the seam for everything a
server tells you on a call that succeeded. A vendor announcing a shutdown does
exactly that — `Deprecation` and `Sunset` ride a `200`, so nothing fails and
nothing retries; [the vendor told you for six months, in a header](/docs/scenarios/deprecation-headers)
finds the three places a response header is actually reachable.
