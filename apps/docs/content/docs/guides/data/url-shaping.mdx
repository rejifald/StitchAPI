...
[`wire.array`](/docs/reference/config-types) — `indices` (the default shown
above), `brackets`, or `repeat`.

The same walker and the same `wire.array` shape a `wire: { body: 'form' }` request
body, so an urlencoded body nests and serializes exactly like the query string:
...
await create({ body: { filter: { type: 'admin' }, ids: [1, 2] } });
// → filter[type]=admin&ids[0]=1&ids[1]=2
```

> [!NOTE]
>
> Query values are percent-encoded with `encodeURIComponent`, so a space in a
> query value goes on the wire as `%20`, not `+` like `URLSearchParams`. A form
> **body** keeps `+`, which is what urlencoded bodies have always used. Both
> decode to a space server-side; see
