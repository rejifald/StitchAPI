...
A cursor is the shape to prefer here. Paging by offset over a collection that is
still being written returns a silently wrong list — items skipped, items
duplicated, every damaged run reporting success — which
[the page that moved while you were reading it](/docs/scenarios/unstable-pagination)
