...
<Callout type="info">
    **`bodyType` is an `http`-surface option.** The `graphql` surface builds its
    own body — a JSON `{ query, variables }` envelope — so setting `bodyType`
    ...
    and `bodyType` are both compile errors on an `llm()` config. (`responseType`
    is not — that one the surface leaves alone, so it still works.)
