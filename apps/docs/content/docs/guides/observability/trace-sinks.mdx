...
| a `TraceSink` | Any custom sink — e.g. from `createTrace` / `multiplex` / `otlp.sink`. |
...
<Callout type="info">
    ...
    `nestLoggerSink`, which forwards the same payload-free metadata to a Nest
...
regardless of the cap. Customer data in a response body is the case this leaves
open: [the customer data you didn't mean to log](/docs/scenarios/pii-in-the-logs)
counts which destinations carry a whole body and which carry none, and why there
