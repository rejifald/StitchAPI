...
build the sink with `otlp.sink()` and combine sinks with `multiplex`:
...
The pipeline is one namespace, `otlp`, with a member per layer.

`otlp.sink(opts?)` returns a sink. `OtlpOptions` carries `endpoint` (OTLP/HTTP
...
`otlp.exporter({ endpoint?, headers? })` is that default exporter — it POSTs
OTLP/JSON to `${endpoint}/v1/traces`. `otlp.json(spans)` serializes collected
