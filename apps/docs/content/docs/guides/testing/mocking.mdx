...
The transport is a [seam](/docs/guides/transport/adapters): a stitch sends through
its `adapter`, defaulting to `fetch`. Pass a mock adapter instead and the rest of
...
transport does (and what [`conformance.adapter`](/docs/reference/conformance-kits)
...
does — a distinction
[the mock that passed for six months](/docs/scenarios/stale-fixture) works
through, along with the drift a fixture can never show you because it never
changes:
