...
description: 'Auth strategies import from stitchapi/auth and the four secret resolvers fold into two namespaces, plus three spec-correct behaviors — lowercase header names, %20 query spaces, and template-narrowed stitch types — that surprise migrators.'
---
...
It opens with two items of a different kind — not conformance surprises but
deliberate pre-GA breaks: the auth strategies moved to their own import, and the
four secret resolvers folded into two namespaces.
...
`cookieSession`) and the secret resolvers (`env`, with `env.optional`, and the
`credential` namespace) now live on the `stitchapi/auth` subpath. Split the
import in two:
...
## The four secret resolvers are two namespaces

Three of the four resolver names are gone from `stitchapi/auth`:

```
Module '"stitchapi/auth"' has no exported member 'secretsFile'.
```

They fold into two namespaces on the same subpath:
...
// @noErrors
// before
import { optionalEnv, secretFrom, secretsFile } from 'stitchapi/auth';
```

```ts twoslash
// after
import { credential, env } from 'stitchapi/auth';
```

Nothing else changes — same resolution order, same empty-value rejection, same
`OptionalSecret` brand, same thunk handed to every strategy. Only the spelling.

**Why two and not one.** These four were never a single cluster. `env` and
`optionalEnv` name the **same source** and differ only in requiredness, so
requiredness becomes a modifier on one callable name: `env('NAME')` throws when
the variable is unset, `env.optional('NAME')` resolves to _absent_ instead.
`secretsFile` and `secretFrom` differ by **source**, so the source is named at
the call site on a namespace that groups the ones that are not the process
environment. Two dimensions, so two names — the same shape as the `duration` /
`size` / `rate` token grammars and the root's `secrets` namespace.

The noun is `credential`, not `secrets`, deliberately: `secrets` is already the
root barrel's trace-redaction namespace (`secrets.register` / `has` / `redact`).
Reusing it here would put two different objects behind one word on two entry
points, which is the one-word-one-concept rule this project holds itself to.

There are no aliases and no deprecation shims. StitchAPI is still pre-GA
(`1.0.0-rc`), and keeping the old spellings would leave two ways to write one
call on the subpath — the thing being removed. The break is a compile error at
the import line, not a runtime surprise.
