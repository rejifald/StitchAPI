...
The five strategies come from the **`stitchapi/auth`** subpath, so a project that
never authenticates does not carry the OAuth2 token dance or the cookie jar in its
bundle. The secret resolvers stay on the root `stitchapi` — they are not
auth-specific, and `@stitchapi/aws-sigv4` takes the same `Secret` values without
ever building a strategy. A typical call site imports from both:
...
Writing your own strategy needs neither: `AuthStrategy` is a type on the root.
...
```ts twoslash
import { secretFrom } from 'stitchapi';
import { bearer } from 'stitchapi/auth';
