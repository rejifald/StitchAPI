...
The surface owns the body encoding, so `bodyType` is not accepted here — setting
...
The surface owns the request shape, so `method` and `responseType` are not
...
[`stitch()`](/docs/guides/authoring/stitch) with `responseType: 'blob'`; the only
thing you give up is the `Content-Disposition` filename parsing.
...
stitch({ url, method: 'POST', responseType: 'blob' });
```
