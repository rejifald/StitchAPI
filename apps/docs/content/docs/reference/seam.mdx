---
title: 'seam()'
description: 'SeamConfig — every prop a seam takes, the eight per-endpoint keys it refuses — plus the Seam handle and the lifecycle-free PrincipalSeam that seam.as() returns.'
...
`seam(options?)` takes a [`SeamConfig`](#seamconfig) fragment and returns a
...
<AutoTypeTable path="../../packages/core/src/types.ts" name="SeamConfig" />

### The eight keys a seam refuses

`SeamConfig` is `StitchConfig` minus
...
**`vault` splits the secrets off by visibility, not by backend.** Auth tokens and
sessions live in the vault, which defaults to a reserved, redacted namespace over
`store`; point `vault` at a KMS-, Vault- or keychain-backed store to harden those
alone. Either may be distributed. It is an ordinary `StitchConfig` prop, so a
standalone `stitch()` takes it too — the seam shares one vault across its members
rather than being the only way to have one.
