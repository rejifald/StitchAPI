...
If you're not in React, you don't need the hook at all: the route emits standard SSE, so a plain browser `EventSource` (or a manual `fetch` + `ReadableStream` reader, when you need POST bodies) consumes it just as well. The same `@stitchapi/next` handler feeds any of them — the streaming hooks for [Vue, Svelte, and Solid](/docs/integrations/react) are the same few lines against their own reactivity primitive.
