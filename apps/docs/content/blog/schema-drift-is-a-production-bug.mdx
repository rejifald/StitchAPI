...
// @noErrors
// By hand you hard-check the required fields yourself — and the soft
// drift (a "42" that should be 42, a key the API just grew) has nowhere
// to surface, so the silent change stays silent.
const getOrder = async (id: number) => {
    ...
    // note is optional; no diff against a contract, so coerced/undeclared
    // changes pass unnoticed.
...
declare const log: { warn(msg: string, meta: { level: string }): void };
declare function renderOrder(order: {
    id: number;
    total: number;
    note?: string;
}): void;
