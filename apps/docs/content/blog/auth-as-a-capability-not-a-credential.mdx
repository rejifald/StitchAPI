...
- **The resolver is only as good as its source.** `env()` and `credential.file()` move the secret out of the committed declaration, but a key in a world-readable `.env` or a leaky CI log is still exposed. The boundary starts where your secret management ends, not before.
