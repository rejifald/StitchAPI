...
- **`each`** — how long any single try gets before it's aborted and (maybe) retried.
...
With retry in the picture, the two timeouts compose cleanly: `each` gates one try, while `total` caps the sum of all tries plus their backoff waits. A long enough `total` is precisely what _allows_ a slow attempt to be retried at all — and a tight `each` is what stops one hung attempt from swallowing the whole budget.
...
async function getQuote(symbol: string) {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
        const each = AbortSignal.timeout(3_000);
        try {
            const res = await fetch(`https://api.example.com/quote/${symbol}`, {
                signal: AbortSignal.any([total, each]),
            });
...
- [Timeouts](/docs/guides/resilience/timeout) — `total` vs `each`, and how they compose with retry.
