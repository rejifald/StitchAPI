...
| `any`      | the **first success** (else all fail) | one output           | **hedging** across redundancy |
| `race`     | the **first to settle** (win or lose) | one output           | **first answer**, win or lose |
...
`any` runs its members concurrently and resolves with the **first to succeed**, ignoring failures until none are left — then it rejects with an `AggregateError` of every failure. The moment one wins, the **losers are aborted**. That's failover across interchangeable sources — and, because every member starts at once and a failure never decides the group, it is the combinator to reach for when you want to **hedge** a latency-sensitive call:
...
`race` resolves or rejects with the first member to **settle** — success _or_ failure — and **cancels the rest**. Use it when the first answer is _the_ answer and a fast failure is a legitimate one — not as a hedge: under `race` a fast failure wins and cancels the member that was about to succeed, which is exactly what a hedge exists to prevent. That job is `any`:
