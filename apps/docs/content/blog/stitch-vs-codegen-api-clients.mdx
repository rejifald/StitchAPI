...
Both approaches give you typed, reusable API access. They make opposite bets about where the truth lives — and the bet is forced by the boundary itself, the one seam in a web app where [you don't own the other end](/blog/the-web-ecosystem-is-missing-a-layer). A code generator like [openapi-typescript-codegen](https://github.com/ferdikoomen/openapi-typescript-codegen) or [Orval](https://orval.dev) bets the spec is accurate and treats it as the source of truth, emitting types and request functions ahead of time. StitchAPI bets that the only thing you can fully trust is the response on the wire, and validates against it on every call. Neither bet is wrong everywhere. Let's walk the axes that actually decide it.
