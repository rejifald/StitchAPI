...
A stitch turns one endpoint into a typed function you call with `await`. Swap the default transport for `xhrAdapter()` and set `wire: { body: 'multipart' }`, and you get upload-progress reporting with the rest of the stitch contract intact:
...
- `wire: { body: 'multipart' }` encodes the body as `multipart/form-data` and sets the boundary header for you.
