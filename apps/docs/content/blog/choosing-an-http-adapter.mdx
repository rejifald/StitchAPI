...
```ts twoslash
import type { AdapterRequest, AdapterResult } from 'stitchapi';

// ---cut---
type Adapter = (req: AdapterRequest) => Promise<AdapterResult>;
```

The request carries everything the transport needs and nothing about policy: `{ url, method, headers, body?, bodyType?, multipart?, array?, response?, stream?, onProgress?, signal? }`. The response is `{ status, headers, body, url? }`, where `body` is the parsed value — JSON when the content type allows, otherwise text — or a `ReadableStream<Uint8Array>` when `stream` was set.
...
Declare one endpoint, leave `adapter` off, and it runs on `fetchAdapter()`; then set the field when a call needs to stream, show upload progress, or reuse an agent. The transport seam has its own guide section — [the adapter seam](/docs/guides/transport/adapters) for the decision, [fetchAdapter](/docs/guides/transport/fetch-adapter), [axiosAdapter](/docs/guides/transport/axios-adapter), and [xhrAdapter](/docs/guides/transport/xhr-adapter) for each transport, and [writing an adapter](/docs/guides/transport/custom-adapter) for the contract — with the signatures in the [helpers reference](/docs/reference/helpers), the canned-response path in the [testing & mocking guide](/docs/guides/testing/mocking), and the primitive everything rides on in [The stitch](/docs/concepts/the-stitch). Coming off axios specifically? See [axios alternatives](/blog/axios-alternatives).
