...
// @noErrors
// By hand, the retry loop, the timeout, and the validation are all
// open-coded inside the handler — and they live nowhere reusable.
export default {
    async fetch(req: Request): Promise<Response> {
        const id = new URL(req.url).searchParams.get('id') ?? '1';
        const retryOn = new Set([429, 502, 503, 504]);

        let res: Response | undefined;
        for (let attempt = 0; attempt < 3; attempt++) {
            const ctrl = new AbortController();
            const t = setTimeout(() => ctrl.abort(), 4000); // timeout.each
            try {
            ...
            if (!retryOn.has(res.status)) break;
            await new Promise((r) => setTimeout(r, 2 ** attempt * 100)); // backoff
        }
        if (!res || !res.ok)
        ...
        // the `as` is a hope — a dropped field is not caught here
        return Response.json(user);
    },
};
```
