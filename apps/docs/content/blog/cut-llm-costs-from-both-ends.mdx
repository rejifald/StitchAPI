...
const search = (q: string) =>
    fetch(`https://api.example.com/search?q=${encodeURIComponent(q)}`).then(
        (r) => r.json(),
    );

// Nothing dedupes, caches, or throttles — ten branches => ten paid calls.
const results = await Promise.all(
    Array.from({ length: 10 }, () => search('stitchapi')),
);
```
