...
For a streaming surface (`sse` / `stream`), `useStitchStream` re-renders as each `delta` chunk arrives. Same result shape; `chunks` is the running list, and `isStreaming` stays true until the terminal `result`:
