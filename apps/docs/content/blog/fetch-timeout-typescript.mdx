...
The two scopes exist because they compose with [retry](/docs/guides/resilience/retry). `each` gates one try, while `total` caps the sum of all tries plus their waits — so a long enough `total` is what _lets_ a slow attempt be retried at all, and a tight `each` stops one hung attempt from swallowing the budget:
