...
- **Calls overlap in time.** Coalescing wins when multiple code paths race to the same endpoint concurrently. Strictly sequential callers never produce an in-flight request to attach to; a fan-out inside a single agent run or a shared route handler is exactly where collapse matters. [One list, a hundred follow-up calls](/docs/scenarios/n-plus-one-fanout) measures that collapse on the classic N+1 — and shows the one thing a coalesced call does not share.
...
There is a second way to stop paying for a repeat read, and the cache primitive can't reach it: ask the server whether anything changed and let it answer `304 Not Modified` with no body at all. That is a conditional request, not a cache hit, and turning the empty response back into the resource takes a seam — [the free poll](/docs/scenarios/conditional-requests-304) walks it end to end.
