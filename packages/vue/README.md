...
`stitchapi` (`^1.0.0-rc.1`) and `vue` (`^3.4`) are peer dependencies. `@tanstack/vue-query` is **not** required — `stitchQueryOptions` returns a plain object.
...
`stitchQueryOptions(stitch, input)` returns a plain `{ queryKey, queryFn }` object — no import of `@tanstack/vue-query` required, so it works even if you never install it. It is implemented once in [`@stitchapi/query-core`](../query-core) and re-exported here, so a stitch keys identically in every framework binding. (Named with the `stitch` prefix because TanStack Query exports its own `queryOptions`.)
