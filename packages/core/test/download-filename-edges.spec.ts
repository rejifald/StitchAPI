...
const filenameFor = async (
    ...
    const adapter = blobAdapter(
        url !== undefined ? { headers, url } : { headers },
    );
...
describe('filenameFromDisposition (fallback branches)', () => {
    it('keeps the raw filename* value when percent-decoding fails', async () => {
        ...
        expect(
            await filenameFor("attachment; filename*=UTF-8''bad%ZZ.txt"),
        ).toBe('bad%ZZ.txt');
    });

    it('parses a filename* that carries a language tag', async () => {
        expect(
            await filenameFor("attachment; filename*=UTF-8'en'report.txt"),
        ).toBe('report.txt');
    });
...
describe('filenameFromUrl (fallback branches)', () => {
    it('percent-decodes the last path segment', async () => {
        expect(
            await filenameFor(
                undefined,
                'https://h.test/files/r%C3%A9sum%C3%A9.pdf',
            ),
        ).toBe('résumé.pdf');
    });
