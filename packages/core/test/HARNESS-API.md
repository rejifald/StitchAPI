...
`bearer(secret)`, `apiKey({ name?, in?, secret })` (`in: 'header' | 'query' | 'cookie'`, default `header`), `basic({ user, pass })`, `cookieSession({ login: <stitch>, cookie: 'sid', credentialsOf?: () => StitchInput, refresh?: [401] })`. Secrets: `env('VAR')` / `credential.file('name')` return `() => string` resolved at call time (and `env.optional('VAR')` returns an `OptionalSecret` that never throws). `cookieSession` auto-logs-in when no cookie is stored, replays the captured cookie, and re-logs-in when a response status is matched by `refresh`.
