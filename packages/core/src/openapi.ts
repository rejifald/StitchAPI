...
// (the `stitch('https://…')` string form composes to `path`). A thunk `url`/`baseUrl` never
// survives redaction (P0), so an absent endpoint here means "never configured OR resolved at call
// time" — either way there is nothing static to export.
...
function endpointOf(cfg: RedactedStitchConfig): ResolvedEndpoint {
    if (cfg.url !== undefined) return { value: cfg.url };
    const base =
        cfg.baseUrl !== undefined ? cfg.baseUrl.replace(/\/+$/, '') : '';
    const path = cfg.path ?? '';
