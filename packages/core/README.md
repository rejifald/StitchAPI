...
- **`timeout` aborts** - `total` and/or `each`, as milliseconds or `'30s'`-style strings, enforced with a real `AbortSignal` instead of a request left hanging.
...
const events = sse({ url: 'https://api.example.com/events' });
...
const getUser = stitch({ path: 'https://api.example.com/users/{id}' });
