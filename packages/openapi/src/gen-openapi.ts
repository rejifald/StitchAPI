...
function emitClient(baseUrl: string | undefined, auth: DerivedAuth): GenFile {
    ...
    // The strategy rides its own subpath import — a client with no auth pulls no strategies.
    if (auth.strategy)
        lines.push(`import { ${auth.strategy} } from 'stitchapi/auth';`);
    lines.push('');
    lines.push(
...
interface DerivedAuth {
    expr?: string;
    /** Names imported from the `stitchapi` ROOT (e.g. `env`). */
    imports: string[];
    /**
     * The strategy factory, imported from the `stitchapi/auth` SUBPATH rather than the root — the
     * strategies live behind a subpath so a generated client that needs no auth never pulls the
     * oauth2/cookieSession machinery into its bundle.
     */
    strategy?: string;
}
