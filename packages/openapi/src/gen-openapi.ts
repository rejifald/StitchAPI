...
function emitClient(baseUrl: string | undefined, auth: DerivedAuth): GenFile {
    const lines: string[] = [];
    lines.push(`import { seam } from 'stitchapi';`);
    // The strategy factory and `env` live on `stitchapi/auth` (ADR 0021), so an authenticated
    // client emits a second import line; an unauthenticated one emits `seam` alone.
    if (auth.authImports.length)
        lines.push(
            `import { ${auth.authImports.join(', ')} } from 'stitchapi/auth';`,
        );
    lines.push('');
    lines.push(
...
interface DerivedAuth {
    expr?: string;
    /** Names the rendered `expr` needs from `stitchapi/auth` — never root exports (ADR 0021). */
    authImports: string[];
}
...
function deriveAuth(
    ...
    if (!schemeName) return { authImports: [] };
    const scheme = schemes[schemeName];
    if (!scheme) return { authImports: [] };

    if (scheme.type === 'http' && scheme.scheme === 'bearer')
        return {
            expr: `bearer(env('API_TOKEN'))`,
            authImports: ['bearer', 'env'],
        };
    if (scheme.type === 'http' && scheme.scheme === 'basic')
        return {
            expr: `basic({ user: env('API_USER'), pass: env('API_PASSWORD') })`,
            authImports: ['basic', 'env'],
        };
    if (
        scheme.type === 'apiKey' &&
        (scheme.in === 'header' ||
