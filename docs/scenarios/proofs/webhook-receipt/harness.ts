...
//
// This scenario's evidence is mostly STATUS CODES and BYTES: what a real `serve` process answered
// when a Stripe-shaped delivery was POSTed at it, and whether the bytes that reach user code are
// still the bytes the provider signed. So both assertions print the measured value whether they
// pass or fail — `404` and `hmac(raw) !== hmac(restringified)` ARE the findings, and they have to
 ...
 * sequence is printed in full whether it passes or fails — the status spine
 * (`[404, 404, 404, 200]`) and the applied-state spine (`["active", "trialing"]`) ARE the evidence.
 */
 ...
 *
 * Several claims here PASS by measuring an ABSENCE — the verdict statement carries the direction,
 * because "PASS C2" on a claim whose content is "there is no such primitive" is otherwise unreadable.
 */
