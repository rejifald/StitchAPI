...
//
// This scenario's evidence is COUNTS and KEY STRINGS: how many of N healthy tenants failed
// because one tenant's credential was revoked, and what string the engine actually keyed the
// breaker / rate bucket on. So both assertions print the measured value whether they pass or
// fail — `9/9 healthy tenants failed` and `circuit:/v1/items` ARE the findings, and they have to
 ...
 * sequence is printed in full whether it passes or fails — the per-tenant outcome spine
 * (`["ok","ok","503","ok"]`) and the arrival-time spine (`[0,100,200]`) ARE the evidence.
 */
 ...
 *
 * Several claims here PASS by measuring a FAILURE of the library — the verdict statement carries
 * the direction, because "PASS C1" on a claim whose content is "one tenant takes down all of them"
