// Ask the COMPILER which per-tenant spellings exist, instead of grepping for them.
//
// "The built-in can't" and "I couldn't find the spelling" are different findings, and only one of
// them is the library's problem. `NoUnknownNestedKeys` (types.ts:411-448) makes an unknown key
// inside a house envelope a compile error naming the slot, so the honest way to establish that
// there is no `throttle.key` / `pool: 'principal'` / `circuit.tenancy` is to hand the compiler
// each candidate spelling and read back its diagnostics. A line that compiles is a spelling that
// EXISTS; a line that doesn't is one the vocabulary refuses.
//
...
export interface ProbeResult extends Candidate {
    compiles: boolean;
    /** First diagnostic code, e.g. 2769 (no overload matches) or 2322 (not assignable). */
    diagnostic?: number;
}
...
export function probeSpellings(
    ...
    const dir = mkdtempSync(join(tmpdir(), 'stitch-tenancy-probe-'));
    const file = join(dir, 'probe.ts');
    const header = [
        `import { seam, stitch } from ${JSON.stringify(CORE)};`,
        `const tenantId: string = 'tenant-42';`,
        `void [seam, stitch, tenantId];`,
    ];
    try {
        writeFileSync(
            file,
