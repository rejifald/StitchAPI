...
//
// This scenario's evidence is a LIST OF ROW IDS: which ids the paginated run skipped, and which it
// returned twice, measured against the ground truth the fake server knows. So every assertion
// prints the measured value whether it passes or fails — `skipped ids: ["r05"]` has to be readable
// out of context, because it IS the finding. `checkSeq` carries most of the weight here: a claim
// about a set of ids is only meaningful if the ids themselves are on the page.
 ...
 * sequence is printed in full whether it passes or fails — the per-call outcome spine
 * (`["ok","ok","400"]`) and the per-provider hit spine (`[1,0]`) ARE the evidence.
 */
 ...
 *
 * Several claims here PASS by measuring DAMAGE — the verdict statement carries the direction,
 * because "PASS C1" on a claim whose content is "the run reported success and lost a row" is
