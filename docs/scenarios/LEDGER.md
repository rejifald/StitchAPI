...
| [`body-verdict-footguns`](issue-drafts/body-verdict-footguns.md) **→ [#651](https://github.com/rejifald/StitchAPI/issues/651)**                                     | high                                            | `verdict.flag` returns `ok: true` on an error envelope; `.safe()` drops `RateLimitError.body`; `backoff` fn silently vanishes                                                                                                                            |
