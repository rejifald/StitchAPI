...
Issue drafts accumulate here and most have since been filed: **18 of the 23 are on GitHub**
(see the table below), **three are held back** as security-sensitive, and **two remain drafted
but unfiled**.

> **2026-08-14 re-measure.** The branch was rebased onto current `main` and every page and proof
> was re-measured against it. The July–August fix wave — much of it fixes to findings this pass
> filed — landed #647, #661, #662, #663, #664, #665, #666, #667 and ADR 0025 (#630), and the
> workspace moved to Zod 4 (#589). Rows below carry ~~strikethrough~~ + a fix reference where a
> recorded finding no longer reproduces; the pages and proofs were updated to pin the current
> behavior.
...
| 5   | A stream that fails after 800 tokens                  | `mid-stream-failure`            | achievable with user code (resumable feeds: **achievable outright**)                                                 | [page shipped](../../apps/docs/content/docs/scenarios/mid-stream-failure.mdx) + 1 issue draft (~~SSE reconnect replays completed streams — a bug in #622~~ fixed by #647)                                        |
...
| 13  | The export that eats the heap                         | `large-response-memory`         | achievable with user code — one seam, ~75 lines; ~~NDJSON only~~ either wire format since #665                       | [page shipped](../../apps/docs/content/docs/scenarios/large-response-memory.mdx) + 1 issue draft (**`.stream()` is not memory-bounded**; ~~`decode: 'json'` buffers~~ fixed by #665)                             |
| 14  | The signature that expired in your own queue          | `expiring-signatures`           | **ACHIEVABLE** — the queue/retry halves need no user code at all                                                     | [page shipped](../../apps/docs/content/docs/scenarios/expiring-signatures.mdx) + 1 issue draft (~~SigV4 ignores the injected clock~~ fixed by #667; a skew 403 opens the breaker)                                |
...
| 16  | One list, a hundred follow-up calls                   | `n-plus-one-fanout`             | achievable with user code — ~8 lines, the partial-failure branch                                                     | [page shipped](../../apps/docs/content/docs/scenarios/n-plus-one-fanout.mdx) + 1 issue draft (a coalesced failure is not shared; ~~a `store` un-pools `pool: 'host'`~~ fixed by ADR 0025)                        |
...
| 18  | The agent picks the arguments                         | `agent-holds-the-tool`          | **credential boundary held** (30 scans, 0 hits); argument boundary is the user's — safe exposure = 3 seams, 47 lines | [page shipped](../../apps/docs/content/docs/scenarios/agent-holds-the-tool.mdx) + 2 issue drafts (unfiltered MCP error channel leaks a query credential; ~~input schemas check but never filter~~ fixed by #663) |
| 19  | The mock that passed for six months                   | `stale-fixture`                 | **split** — resilience/streams test perfectly offline; the scenario's own direction is **invisible** offline         | [page shipped](../../apps/docs/content/docs/scenarios/stale-fixture.mdx) + 1 issue draft (~~`manualClock` covers 6 of 12; 2 bugs in the testing kit~~ all fixed by #664, table re-measured)                      |
| 20  | The ID that changed on the way in                     | `precision-loss`                | **achievable in config** — `wire.response` + `transform` = 16 lines, exact digits; the default corrupts silently     | [page shipped](../../apps/docs/content/docs/scenarios/precision-loss.mdx) + 1 issue draft (~~a `bigint` in `params` silently vanishes while `query` handles it~~ fixed by #661)                                  |
...
Eighteen are filed (#640–#645, #648, #650, #651–#660). **Three are deliberately held back** as
security-sensitive — see the note below. Two more are drafted but unfiled:
`bigint-in-params-vanishes` (its defect was fixed by #661 before filing — superseded) and
`diff-primitives-are-unreachable` (still valid, still unfiled).
...
| [`body-verdict-footguns`](issue-drafts/body-verdict-footguns.md) **→ [#651](https://github.com/rejifald/StitchAPI/issues/651)**                                     | high                                                                      | `verdict.flag` returns `ok: true` on an error envelope; ~~`.safe()` drops `RateLimitError.body`~~ (fixed by #662); ~~`backoff` fn silently vanishes~~ (fixed by #666 — now throws `bad backoff` at construction)                                                                                                              |
...
| [`sse-reconnect-replays-completed-streams`](issue-drafts/sse-reconnect-replays-completed-streams.md) **→ [#640](https://github.com/rejifald/StitchAPI/issues/640)** | ~~highest — a bug in freshly shipped #622~~ **fixed by #647**             | ~~`sse: { reconnect: true }` reopens a **completed** id-less stream 4× and delivers `ABCDEABCDEABCDEABCDE` to the consumer, ending `ok: true`~~ (fixed by #647 — a cleanly-finished stream is never reopened; issue closed)                                                                                                   |
...
| [`streaming-is-not-memory-bounded`](issue-drafts/streaming-is-not-memory-bounded.md) **→ [#659](https://github.com/rejifald/StitchAPI/issues/659)**                 | **high — §1 open; §2 fixed by #665**                                      | the engine's `chunks` accumulator retains every delta so `.stream()` measures the same as `await` (§1, still open); ~~`decode: 'json'` buffers the array it streams, tripping its own cap at 37k rows~~ (§2, fixed by #665 — the cap now bounds one element)                                                                  |
| [`sigv4-ignores-the-injected-clock`](issue-drafts/sigv4-ignores-the-injected-clock.md) **→ [#658](https://github.com/rejifald/StitchAPI/issues/658)**               | medium — clock half **fixed by #667**                                     | ~~SigV4 signs with `new Date()` so skew is untestable on a virtual clock~~ (fixed by #667 — the signer stamps from `ctx.clock`); a skew 403 opens the dependency's breaker; `onRequest` runs after signing                                                                                                                    |
...
| [`coalescing-does-not-share-failures`](issue-drafts/coalescing-does-not-share-failures.md) **→ [#653](https://github.com/rejifald/StitchAPI/issues/653)**           | medium (+ a strong positive)                                              | in-flight coalescing genuinely works (100 calls / 30 ids → 30 requests), but a coalesced FAILURE releases every joiner — 100 requests for one 404ing id; ~~and a `store` silently un-pools `pool: 'host'` concurrency~~ (fixed by ADR 0025/#630 for lease-capable stores; lease-less stores still per-process)                |
...
| [`input-schemas-check-but-never-filter`](issue-drafts/input-schemas-check-but-never-filter.md) **→ [#648](https://github.com/rejifald/StitchAPI/issues/648)**       | ~~high~~ **fixed by #663, issue closed**                                  | ~~`validateInput` discards the parsed value while `validateOutput` returns it, so a stripping schema — the Zod/Valibot/ArkType default — does not strip~~ (fixed by #663 — a declared slot's parsed value now shapes the request; undeclared slots stay passthrough)                                                          |
| [`testing-kit-clock-gaps-and-two-bugs`](issue-drafts/testing-kit-clock-gaps-and-two-bugs.md) **→ [#650](https://github.com/rejifald/StitchAPI/issues/650)**         | ~~high~~ **all three fixed by #664 — issue still open, should be closed** | ~~`manualClock` drives 6 of 12 time-driven features (OAuth2 expiry is NEW and undocumented); `stubStitch().safe()` throws on a sync throw; `mockAdapter` violates the library's own `abort` rule~~ (all fixed by #664: OAuth2 expiry rides the injected clock, the stub resolves async, the mock rejects pre-aborted signals) |
| [`bigint-in-params-vanishes`](issue-drafts/bigint-in-params-vanishes.md)                                                                                            | ~~medium-high~~ **superseded — fixed by #661 before filing**              | ~~`expandTemplateVar` omits `bigint` while `stringifyLeaf` includes it, so the same id is exact in `query` and gone in `params`~~ (fixed by #661 — bigint joined the scalar arm; path and query now agree); the `.report()`-under-bigint-body note was not re-verified                                                        |
...
>    _2026-08-14: the stock-Zod repro died with the Zod 4 bump (#589 — messages no longer echo
>    the received value); the `drift.ts` verbatim-copy mechanism persists (re-proved in C7(h))
>    and the ask stands, scoped to validators whose messages echo._
>
...
>    a genuine bug in code that shipped in **#622**.~~ **Resolved:** filed as #640, fixed by
>    #647 — a cleanly-finished stream is never reopened. Nothing left to triage here.

### Patterns across the pass

**0. ~~One stale docs reference, verified and left unfixed.~~ Since fixed on `main`.**
`run-identity.mdx:26`/`:33` described "each step of a `pipe()`" where the construct is
`linked()`; the two-line edit was deliberately left out of the scenario commits and has since
landed — the file now says `linked()` in both places. Nothing left to do.
...
**2b (re-measured 2026-08-14).** The audit happened: #664 put OAuth2 expiry (and the two
testing-kit bugs) on the injected clock and #667 did the same for SigV4 signing. The
still-wall-clock set is what remains in `stale-fixture.mdx`'s re-measured table — that table is
now the canonical count, not the six-vs-six above.
...
its job. It is one line (the `items.length === 0 || page >= max` break in `paginated()`,
engine.ts:1015 as of the 2026-08-14 rebase) and it is the single most expensive default found in
this pass.
