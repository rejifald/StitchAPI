...
12. **No side effects by default.** **[decided]** A stitch's call is its only effect on the world. The state it keeps — throttle buckets, cookie jar, token cache, circuit-breaker counters — is in-memory and process-local, gone when the process exits; nothing is persisted or shared across workers unless you attach a `store` (§13). Distributed rate-limiting and persistent/shared sessions are a deliberate opt-in, never a silent default — progressive disclosure applied to durability.
