...
Commit b06cf00's rebuild silently dropped three documented capabilities because no regression net pinned them. Most of the exposure this finding listed has since been closed (see Resolved); two holes remain, both on the retry path.

**Transport-error retry is unpinned.** Every retry spec drives off a _status_ — 503, `Retry-After`, exhaustion (`resilience.spec.ts:36-132`) — and none makes the transport itself throw, so the `catch` that turns a network error into a retry (`engine.ts:675-700`) is exercised by nothing. That is the arm b06cf00-class rebuilds drop most quietly, because a status-driven suite stays green without it.

**The `onError` / `onRetry` hooks are unpinned.** They appear in the suite only as no-op functions inside a `contract-p0.spec.ts:80-81` envelope-shape assertion — never asserted to fire, to receive the right context, or to chain across composed fragments — although the engine calls them from five sites (`engine.ts:676,689,759,793,1354`) and hook chaining through `extends` is a documented authoring feature.

Separately, the OTLP path is now tested but still **swallows every export failure with no diagnostic outlet** (`otlp.ts:160,163,371`), so a misconfigured collector is indistinguishable from a working one.
**Build:** a spec whose adapter throws, asserting the retry fires and the reported attempt count; a hooks spec asserting `onError`/`onRetry` fire with the right context and chain across `extends`; and a one-time stderr warning on first OTLP export failure (note `STITCH_TRACE_CONSOLE` cannot reveal export failures — a new mechanism is needed).
**Resolved (2026-08-05):** the rest of this finding no longer holds. Backoff curves (`expo`/`fixed`), the backoff `max` ceiling (audited as `maxMs`, since folded into the `backoff` envelope), jitter bounds, and `Retry-After` as an HTTP-date — including a past date clamping to 0 — are covered by the dedicated `resilience-backoff.spec.ts`; `timeout.each` (audited as `perAttempt`, renamed in #627) is exercised by nine spec files; the CLI `main()` dispatch is driven with injected IO across `cli-init`, `openapi`, `diagram`, `from-curl` and `gaps/cli-since-exit`, the last of which also reaches `traceCommand`; the `--since` bug is fixed exactly as asked — `cli.ts:576-586` parses the value and returns exit 2 with `invalid --since value: …` instead of silently setting `cutoff = now`, pinned by `gaps/cli-since-exit.spec.ts`; and `otlpHttpExporter` / `toOtlpJson` have `otlp-export.spec.ts` (six cases, including an end-to-end export to a stub exporter and URL-credential redaction) and `otlp-json.spec.ts`.
