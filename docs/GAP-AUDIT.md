...
### 1.1 `timeout.total` is not a total deadline — and is dead config in the documented example — RESOLVED

The docs promise a hard cross-retry budget ("`total` caps the sum of all tries plus their backoff waits"), but `engine.ts:270-272` computes the per-attempt cap as `timeout.each ?? timeout.total` (`each` was then spelled `perAttempt`): `total` is only a per-attempt fallback, and when both knobs are set (the docs' own examples) `total` is read nowhere at all. There is no cumulative deadline anywhere — backoff sleeps (`engine.ts:318,356`), throttle waits, and pagination loops are unbounded, so `retry.attempts: 5` + `total: '30s'` can run for minutes; the only test (`packages/core/test/resilience.spec.ts:146-160`) is single-attempt, where the two semantics are indistinguishable.
**Build:** a wall-clock deadline in `execute()`/`attemptLoop` (clamp each attempt's signal to `min(each, remaining)`, count backoff/throttle waits, also cover `paginated()` and `executeRaw()`), plus specs pinning both knobs. The docs already define the desired semantics, so implement rather than rewrite.
**Resolved (2026-08-04):** built as specified. `totalBudget()` derives one shared deadline from `t0` (`engine.ts:453-466`) and it is threaded through every waiting path: `sleepWithin` for retry/reconnect backoff, `acquireWithin` for throttle and rate waits, the attempt loop (each attempt's abort clamped to `min(timeout.each, remaining)`), the pagination loop, the streaming/SSE paths, and `executeRaw`. A breach fails with `TimeoutError('timed out after Nms')` rather than running long. Pinned by `packages/core/test/gaps/timeout-total.spec.ts` — five cases covering the backoff loop, both knobs set together, a throttle rate wait, a paginated call, and `executeRaw` — plus `resilience.spec.ts:132` for a long `Retry-After`.
**Evidence (as audited):** `packages/core/src/engine.ts:270-272`, `packages/core/src/resilience.ts:124-146`, `apps/docs/content/docs/guides/resilience/timeout.mdx:22-39`, `apps/docs/content/docs/errors/stitch-timeout.mdx:8-50`, `packages/core/README.md:336`.
...
Commit b06cf00's rebuild silently dropped three documented capabilities because no regression net pinned them. Most of the exposure this finding listed has since been closed (see Resolved); two holes remain, both on the retry path. **Transport-error retry is unpinned:** every resilience spec drives retries off a _status_ (503, `Retry-After`), and none makes the transport itself throw, so the `catch` that turns a network error into a retry (`engine.ts:675-700`) is exercised by nothing. **The `onError` / `onRetry` hooks are unpinned:** they appear in the suite only as no-op functions inside a `contract-p0.spec.ts:80-81` envelope-shape assertion — never asserted to fire, to receive the right context, or to chain across composed fragments, even though the engine calls them from five sites (`engine.ts:676,689,759,793,1354`). Separately, the OTLP path is now tested but still swallows every export failure with no diagnostic outlet (`otlp.ts:160,163,371`), so a misconfigured collector stays silent.
**Build:** a spec whose adapter throws, asserting the retry fires and the attempt count; a hooks spec asserting `onError`/`onRetry` fire with the right context and chain across `extends` fragments; plus a one-time stderr warning on first OTLP export failure (note `STITCH_TRACE_CONSOLE` cannot reveal export failures — a new mechanism is needed).
**Resolved (2026-08-04):** the rest of this finding no longer holds. Backoff curves (`expo`/`fixed`), the backoff `max` ceiling (audited as `maxMs`, since folded into the `backoff` envelope), jitter bounds, and `Retry-After` as an HTTP-date (including a past date clamping to 0) are covered by the dedicated `resilience-backoff.spec.ts`; `timeout.each` (audited as `perAttempt`) is exercised by nine spec files; the CLI `main()` dispatch is driven with injected IO across `cli-init`, `openapi`, `diagram`, `from-curl` and `gaps/cli-since-exit`, which also reaches `traceCommand`; the `--since` bug is fixed exactly as this finding asked — `cli.ts:576-586` now parses the value and returns exit 2 with `invalid --since value: …` instead of silently setting `cutoff = now`, pinned by `gaps/cli-since-exit.spec.ts`; and `otlpHttpExporter` / `toOtlpJson` have `otlp-export.spec.ts` (six cases, including an end-to-end export to a stub exporter and URL-credential redaction) and `otlp-json.spec.ts`.
