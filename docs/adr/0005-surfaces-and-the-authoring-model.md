...
1.  **A surface is a plugin, not an enum member.** Introduce `Surface<TInput, TResult>` — an object with a stable string `id` and a small set of **behaviour hooks** that the engine calls instead of branching on `kind`. The minimum hooks cover the three things the engine special-cases today and the two things streaming adds:
    ...
    - `buildRequest(cfg, input) => AdapterRequest` (or a patch over the http default) — how the call is shaped. `graphql` puts `{ query, variables }` in the body and forces `POST`; `download` forces `GET` + `response: 'blob'`; `http` is the identity.
    ...
    **Addendum (2026-08-03) — what a surface owns, the caller may not author.** "A surface owns shaping" has a corollary this decision left implicit: any config field the surface's `buildRequest` sets unconditionally is **not** a knob, and authoring it must be a **compile error**, not a value that is quietly discarded. `graphqlSurface.buildRequest` hardcodes the transport-level `bodyType: 'json'` on the `AdapterRequest` it returns, so the config-level `{ kind: graphql, wire: { body: … } }` was silently dead config; `NoWireBodyOnGraphql` / `WireBodyFixedByGraphql` now reject it on every authoring surface (`stitch` — both overloads — `graphql()`, `Seam.stitch`, `Seam.graphql`). Because the sibling `MultipartOnlyOnMultipartBody` guard already requires `wire.body: 'multipart'` before `wire.multipart` is legal, one guard closes both dead pairings on graphql. Only the body is claimed — `wire.response` and `wire.array` are not body encodings and stay legal. The guard is compile-time only, so `buildRequest`'s override stays deterministic and is pinned by a test — a config rebuilt at runtime (a deserialised `__config`, `fromCurl`) can still carry the field.

    Like the sibling guards, it reads the **composed** config through `Layers`, so a surface inherited via `extends` counts. One asymmetry is worth recording: the surface probe here is an **inhibitor**, not an enabler. For `MultipartOnlyOnMultipartBody` and `GraphqlOnlyOnGraphqlSurface`, finding the enabler on some layer makes a config legal, so `AnyLayer`'s existential scan can only fail open — the property that justified choosing it over last-wins resolution. Finding the graphql surface makes a config _illegal_, so the same scan fails CLOSED here: a config that inherits graphql and then overrides `kind` back to a non-graphql surface is rejected although its `wire.body` is live. Accepted rather than resolved — distinguishing it requires the last-wins resolution the existential scan exists to avoid, and the config it costs is perverse (inherit a GraphQL base, then make it not GraphQL) with an obvious workaround. Pinned as a tsd expectation so the tradeoff stays visible.

    **`wire.body: 'multipart'` on graphql is REJECTED, not implemented — GraphQL file uploads are out of scope for 1.0.** Supporting them means the [GraphQL multipart request spec](https://github.com/jaydenseric/graphql-multipart-request-spec) (`operations` + `map` + one part per file), which cannot arrive as a `wire.body` value: the surfaces are mutually exclusive through the single `kind` slot, so multipart cannot be its own surface and the envelope would have to be learned by `graphqlSurface.buildRequest` itself. Deferred deliberately, on four grounds:

    - **No demand, wrong phase.** Zero prior mention in the repo, at 1.0.0-rc; the current work is a pre-GA naming/stabilisation sweep, not surface expansion.
    - **The ecosystem standard is receding.** Apollo Server dropped built-in `graphql-upload` integration in v3 (2021) and now documents manual middleware plus a CSRF warning; Apollo's own best-practice guidance steers to signed-URL direct uploads.
    - **It re-creates the defect one level down.** The spec fixes the envelope, so the nesting grammar would be inert on graphql — `{ kind: graphql, wire: { body: 'multipart', multipart: 'dot' } }` would satisfy `MultipartOnlyOnMultipartBody` and still be ignored, needing a further guard.
    - **The encoding is not free.** The spec requires `operations` and `map` to **precede** the file parts (servers stream-parse them), but JS orders integer-like object keys first, so the natural `{ operations, map, '0': file }` body serialises as `0, 1, operations, map`. Landing it means either non-numeric part names (spec-legal, some server-compat risk) or teaching the adapter to accept a pre-built `FormData` — an ADR-sized choice, not a bug fix.

    Relaxing the guard later is **non-breaking**, so the door stays open: implementing the spec is a scoped follow-up, not a migration.

    **Addendum (2026-08-03, cont.) — the same sweep across the other surfaces.** The rule above is general, so it was applied to every surface whose `buildRequest` overwrites a caller-authorable field, not just graphql:

    - **`download`** forces `method: 'GET'` and a blob response. `NoRequestShapeOnDownload` / `RequestShapeFixedByDownload` reject `method` and `wire.response`. The response decoding is the load-bearing one — `interpret` casts `res.body` to a `Blob` before returning `{ blob, filename }`, so any other response type makes that cast a lie. `method` is weaker (POST-then-download is a real pattern), but honouring it alone would leave the surface's own name describing half the request; the escape hatch is a plain `stitch()` with `wire: { response: 'blob' }`, costing only the `Content-Disposition` filename parsing. Relaxing `method` later is non-breaking. Note the two halves of this guard sit at different **depths** — `method` is a flat slot, `wire.response` a nested one — and the nested arm names the single member rather than the envelope, so `wire.array` and `wire.multipart` stay authorable on a download stitch. `RequestShapeFixedByDownload`'s surface probe is an **inhibitor**, exactly like `WireBodyFixedByGraphql`'s above, so it inherits the same fail-CLOSED polarity for the same reason and on the same terms; both are pinned in tsd.
    - **`llm`** forces `method: 'POST'` and a JSON body. `NoRequestShapeOnLlm` rejects `method` and `wire.body`, leaving `wire.response` live (`buildRequest` never touches it). It is a plain object type rather than a conditional over `C`, because `llm()` infers nothing from its config (it returns a flat `Stitch<LlmResult>`) and so keeps a **non-generic** parameter — which is what preserves excess-property checking, and excess-property checking is what makes the removed `maxTokens` spelling an error (P4). Adopting the conditional idiom here would have silently traded that guarantee for this one. There is deliberately no `kind`-keyed sibling: the exported `llmSurface` is only the redaction identity and carries no `buildRequest`, so `method` **is** live on `stitch({ kind: llmSurface })` and guarding it there would reject config that works.

    Three surfaces were checked and deliberately **not** guarded, because the deadness has a different cause and a different blast radius:

    - **`sse` / `stream`** have no `buildRequest` at all, so `method` passes through and is genuinely honoured (a POST that returns `text/event-stream` is real). Their `wire.response` is inert, but that is the **engine**'s doing — `runStreaming` sets `stream: true` on the request and the adapter hands back the live `ReadableStream` before it ever consults the response type. That is one rule about the streaming path applying to _any_ surface with a `stream` hook, third-party ones included, not a per-surface override; guarding it belongs to a decision about `stream: true`, not to this sweep.
    - **`postmessage`** discards `method` / `wire.body` / `wire.response` and most other HTTP knobs, but via a custom `execute` that replaces the transport entirely (ADR 0008) — there is no HTTP request to shape. The honest fix there is narrowing what `RequestOptions` / `EmitOptions` / `EventsOptions` admit in the first place, rather than bolting per-field guards onto a `Partial<Omit<StitchConfig, …>>`; that is a larger, separable change.
...
6.  **Nested multipart — `multipart.nesting`.** `encodeRequestBody` / `appendForm` ([`http-adapter.ts`](../../packages/core/src/http-adapter.ts)) currently iterate top-level keys only, so a nested object becomes `[object Object]`. Add `multipart.nesting`:
    ...
    **Addendum (2026-08-03) — the `form` arm, and the `wire` envelope.** As written, this decision fixed `[object Object]` for `multipart` only. The `form` arm of `encodeRequestBody` had the identical defect and no escape hatch: it flattened top-level keys with `String(v)`, so a nested object became `[object Object]` and an array was comma-joined regardless of the configured array format. Both urlencoded surfaces — the query string and a form body — now run **one** walker (`flattenParams` in `util.ts`), so one setting governs both and nesting expands `qs`-style on each. The two differ only in how a space is spelled (`%20` in a query via `encodeURIComponent`, `+` in a body via `URLSearchParams`), which is preserved on both sides. Form arrays therefore default to `'indices'`, matching the query string; the previous comma-join was undocumented and untested. `multipart` keeps its own `nesting` grammar — the encodings are not symmetric, which is why the array setting is shared rather than folded into a per-encoding envelope (CONTRACT.md P24 carve-out (b)).

    The authoring fields this decision introduced flat — `bodyType` and `multipart` — moved into the **`wire`** envelope alongside the response decoding and the array format, which is the shape they are spelled in everywhere above from this point on (`wire: { body: 'multipart', multipart: 'dot' }`). `AdapterRequest` keeps those fields FLAT (`bodyType` / `multipart` / `array` / `response`) and the engine converts depth when it builds the request. The 2026-09-04 sweep took the last foreign spelling off it: `arrayFormat`→`array` and `responseType`→`response`, since the values were already house-normalised and every adapter converts at its own edge (P18/P24 (a)) — the XHR spelling survives on `XhrLike`, the duck-type that meets XHR.
...
Q4 left `output` unapplied to a streamed body, reasoning that validation is a buffered-response concept. Revisited: that is right for the _assembled array_ but wrong for the _individual value_. A streaming surface's delivery unit is the `delta`, not a whole body — so the contract belongs on the delta, and checking it there actually gates what a `.stream()` consumer sees (validating the collected array could only fire _after_ every delta had already been delivered, so it would protect only the bounded-`await` form — the least streaming-shaped use of a streaming surface).

**Decision.** On a streaming surface, `output` validates **each `delta` value before it is emitted**, reusing the engine's existing [`validateOutput`](../../packages/core/src/engine.ts) and its drift leveling: a value that violates a `critical` path fails the stream (a fatal `error`, as on the buffered path), a `watch` path emits a `drift` warning and the delta still flows. `transform` / `unwrap` stay unapplied — they reshape a single whole body, which a stream does not have. Snapshot-drift (`DriftSpec.snapshotFile`) is **not** applied per-delta: a baseline of one record is meaningless against a variable-length stream, so only the schema + `watch` / `critical` leveling apply. The _assembled array_ is still never validated as a unit (its length and content vary per call; a `DriftSpec` snapshot would false-positive every run).

> [!NOTE]
>
> **Superseded in part by [ADR 0015](./0015-schema-anchored-drift.md).** `DriftSpec.snapshotFile` and the `critical` / `watch` leveling referenced above were removed when drift became schema-anchored. Per-delta drift is now plain schema validation (with the optional `raw`-vs-`validated` diff applied per emitted value). The conclusion is unchanged: a snapshot baseline was never meaningful against a stream, and the assembled array is still not validated as a unit.

**Why `output`, not a new key.** A streaming `await` already resolves to an array of the delta type (`SseEvent[]`, `unknown[]`), so `output` describing that element type is consistent with the surface's own typing; it round-trips as JSON like every other contract (the contract-not-dependency gate) and reuses the `drift` event channel rather than inventing a second one. The accepted cost: `output` means "the whole body" on a buffered surface but "each emitted value" on a streaming one — acceptable because a streaming surface has no single body for the buffered meaning to apply to.

**Per-surface detail.** For `stream` (`ndjson` / `lines`) the `delta` _is_ the value, validated directly. For `sse` the `delta` is the `SseEvent` envelope, so `output` validates its `data` payload (the envelope shape is fixed by the surface). For `bytes` (and the raw `lines` decoder) a delta is a `Uint8Array` / `string` fragment that no schema sensibly covers — `output` is normally absent there and validation is a no-op.

**Framing vs. structure (scope).** The two _framed_ decoders already produce complete values — `ndjson` rejoins a record split across chunks in [`lineReader`](../../packages/core/src/line-reader.ts) before `JSON.parse`, and `sse` accumulates `data:` lines until the blank-line dispatch — so per-`delta` validation needs no new parsing. This is **Layer 1**. Accumulating until a value is _structurally_ complete (balanced braces, no newline / event framing) — a streaming JSON parser for **unframed** payloads — is a separate decoder, deferred as **Layer 2** ([#111](https://github.com/rejifald/StitchAPI/issues/111)).

**Status.** Decided; supersedes the original final sentence of Q4. Layer 1 (per-`delta` `output` validation for `ndjson` / `sse`, via the surface's `contractValue` hook) is **implemented**. Typed delta arrays — inferring the element type from `output` (`sse()` → `Stitch<SseEvent<T>[]>`, `stream()` → `Stitch<T[]>`) — are a compile-time follow-up ([#115](https://github.com/rejifald/StitchAPI/issues/115)). Layer 2 (the unframed structural `decode: 'json'` parser) is deferred to [#111](https://github.com/rejifald/StitchAPI/issues/111).
