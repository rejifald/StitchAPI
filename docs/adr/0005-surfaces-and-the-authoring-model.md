...
1.  **A surface is a plugin, not an enum member.** Introduce `Surface<TInput, TResult>` — an object with a stable string `id` and a small set of **behaviour hooks** that the engine calls instead of branching on `kind`. The minimum hooks cover the three things the engine special-cases today and the two things streaming adds:
    ...
    **Addendum (2026-08-03) — what a surface owns, the caller may not author.** "A surface owns shaping" has a corollary this decision left implicit: any config field the surface's `buildRequest` sets unconditionally is **not** a knob, and authoring it must be a **compile error**, not a value that is quietly discarded. `graphqlSurface.buildRequest` hardcodes a JSON body, so `{ kind: graphql, wire: { body: … } }` was silently dead config; `NoBodyTypeOnGraphql` / `BodyTypeFixedByGraphql` now reject it on every authoring surface (`stitch` — both overloads — `graphql()`, `Seam.stitch`, `Seam.graphql`). Because the sibling `MultipartOnlyOnMultipartBody` guard already requires `wire.body: 'multipart'` before `wire.multipart` is legal, one guard closes both dead pairings on graphql. The guard is compile-time only, so `buildRequest`'s override stays deterministic and is pinned by a test — a config rebuilt at runtime (a deserialised `__config`, `fromCurl`) can still carry the field.

    Every guard in this addendum reads the **authoring** spelling, which is the `wire` envelope after Decision 6's addendum below. The `AdapterRequest` these `buildRequest` hooks return is unchanged and keeps the flat `bodyType` / `responseType` (P22 — the XHR vocabulary belongs to the layer that meets XHR), so the flat spellings elsewhere in this decision describe the transport contract, not a stale config field.
    ...
    **`wire.body: 'multipart'` on graphql is REJECTED, not implemented — GraphQL file uploads are out of scope for 1.0.** Supporting them means the [GraphQL multipart request spec](https://github.com/jaydenseric/graphql-multipart-request-spec) (`operations` + `map` + one part per file), which cannot arrive as a `wire.body` value: the surfaces are mutually exclusive through the single `kind` slot, so multipart cannot be its own surface and the envelope would have to be learned by `graphqlSurface.buildRequest` itself. Deferred deliberately, on four grounds:
    ...
    - **It re-creates the defect one level down.** The spec fixes the envelope, so `wire.multipart.nesting` would be inert on graphql — `{ kind: graphql, wire: { body: 'multipart', multipart: 'dot' } }` would satisfy `MultipartOnlyOnMultipartBody` and still be ignored, needing a further guard.
