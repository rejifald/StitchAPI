...
> [!NOTE]
>
> **Superseded in part by [ADR 0013](./0013-schema-anchored-drift.md).** `DriftSpec.snapshotFile` and the `critical` / `watch` leveling referenced above were removed when drift became schema-anchored. Per-delta drift is now plain schema validation (with the optional `raw`-vs-`validated` diff applied per emitted value). The conclusion is unchanged: a snapshot baseline was never meaningful against a stream, and the assembled array is still not validated as a unit.
