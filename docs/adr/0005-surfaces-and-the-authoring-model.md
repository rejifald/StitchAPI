...
1.  **A surface is a plugin, not an enum member.** Introduce `Surface<TInput, TResult>` — an object with a stable string `id` and a small set of **behaviour hooks** that the engine calls instead of branching on `kind`. The minimum hooks cover the three things the engine special-cases today and the two things streaming adds:
    ...
    **Addendum (2026-08-03) — what a surface owns, the caller may not author.** "A surface owns shaping" has a corollary this decision left implicit: any config field the surface's `buildRequest` sets unconditionally is **not** a knob, and authoring it must be a **compile error**, not a value that is quietly discarded. `graphqlSurface.buildRequest` hardcodes `bodyType: 'json'`, so `{ kind: graphql, bodyType: … }` was silently dead config; `NoBodyTypeOnGraphql` / `BodyTypeFixedByGraphql` now reject it on every authoring surface (`stitch` — both overloads — `graphql()`, `Seam.stitch`, `Seam.graphql`). Because the sibling `MultipartOnlyOnMultipartBody` guard already requires `bodyType: 'multipart'` before `multipart` is legal, one guard closes both dead pairings on graphql. The guard is compile-time only, so `buildRequest`'s override stays deterministic and is pinned by a test — a config rebuilt at runtime (a deserialised `__config`, `fromCurl`) can still carry the field.

    **`bodyType: 'multipart'` on graphql is REJECTED, not implemented — GraphQL file uploads are out of scope for 1.0.** Supporting them means the [GraphQL multipart request spec](https://github.com/jaydenseric/graphql-multipart-request-spec) (`operations` + `map` + one part per file), which cannot arrive as a `bodyType` value: the surfaces are mutually exclusive through the single `kind` slot, so multipart cannot be its own surface and the envelope would have to be learned by `graphqlSurface.buildRequest` itself. Deferred deliberately, on four grounds:
    ...
    - **It re-creates the defect one level down.** The spec fixes the envelope, so `multipart.nesting` would be inert on graphql — `{ kind: graphql, bodyType: 'multipart', multipart: 'dot' }` would satisfy `MultipartOnlyOnMultipartBody` and still be ignored, needing a further guard.
    ...
    **Addendum (2026-08-03, cont.) — the same sweep across the other surfaces.** The rule above is general, so it was applied to every surface whose `buildRequest` overwrites a caller-authorable field, not just graphql:
    ...
    - **`sse` / `stream`** have no `buildRequest` at all, so `method` passes through and is genuinely honoured (a POST that returns `text/event-stream` is real). Their `responseType` is inert, but that is the **engine**'s doing — `runStreaming` sets `stream: true` on the request and the adapter hands back the live `ReadableStream` before it ever consults `responseType`. That is one rule about the streaming path applying to _any_ surface with a `stream` hook, third-party ones included, not a per-surface override; guarding it belongs to a decision about `stream: true`, not to this sweep.
    - **`postmessage`** discards `method` / `bodyType` / `responseType` and most other HTTP knobs, but via a custom `execute` that replaces the transport entirely (ADR 0008) — there is no HTTP request to shape. The honest fix there is narrowing what `RequestOptions` / `EmitOptions` / `EventsOptions` admit in the first place, rather than bolting per-field guards onto a `Partial<Omit<StitchConfig, …>>`; that is a larger, separable change.
