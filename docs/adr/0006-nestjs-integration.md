...
- **Exception mapping delivered** (was Decision 10 non-goal) — `StitchExceptionFilter` plus `stitchError.map()` / `stitchError.is()` map a `StitchError` to an `HttpException` (the two helpers were `toHttpException()` / `isStitchError()` until the 2026-08-28 namespace fold — CONTRACT.md §6; the filter stays a class, per ADR 0012 rule 1). Status is **`502 Bad Gateway` by default** (never leaks an upstream's status to the client); configurable via `status: number | (err) => number` (propagate, fix, or remap).
