...
- **Status:** Accepted (2026-07-31). **Supersedes [ADR 0020](./0020-declarative-auth-descriptors.md)** — the declarative `AuthDescriptor` intake is withdrawn before implementation. Salvages 0020's Q7 (`auth` as an atomic `extends` slot) as an independent bug fix, and keeps [#485](https://github.com/rejifald/StitchAPI/pull/485)'s symmetric `apiKey({ in, name, value })` unchanged.
- **Date:** 2026-07-31
...
- **The secret resolvers move with the factories.** Every use of `env()` in the
  ...
  and costs 0.27 KB gzip on the root entry (22.93 vs 22.66 — measured below).
...
win** — the whole entry stays at 24.99 KB either way, because the root barrel would
...
Salvaged verbatim from 0020's Q7, and independent of everything above — **this is a
live bug on `main` today.** `auth` flows through `deepMerge`, which blends two
strategies field-by-field. Verified against the built `lib/`:
...
Same harness, `lib/` built at `main` (`d0f8f12`), esbuild + gzip -9, matching
`scripts/bundle-size.mjs`:

| scenario                                 | min      | gzip         | note                        |
| ---------------------------------------- | -------- | ------------ | --------------------------- |
| `stitchapi` whole entry — today          | 69.76 KB | **24.99 KB** | budget 25.10, headroom 0.11 |
| `stitchapi` whole entry — after the move | 63.43 KB | **22.66 KB** | **−2.33 KB**                |
| `import { stitch }` — today and after    | 55.86 KB | **20.14 KB** | **unchanged**               |
| `stitchapi/auth` — whole surface         | 13.10 KB | 5.11 KB      | new subpath scenario        |
| `stitchapi/auth` — `bearer` + `env`      | 0.78 KB  | 0.46 KB      |                             |
| `stitchapi/auth` — `oauth2` + `env`      | 7.81 KB  | 3.28 KB      |                             |

The auth surface weighs **2.33 KB gzip**. Descriptors force it onto everyone's lean
path; the subpath takes it off everyone's whole entry. Same bytes, opposite sign.
...
advertise — is what that difference is worth, and it is 2.33 KB.

Headroom also stops being a problem. The gate holds ~0.2 KB; the current 0.11 KB is
already a squeeze taken in #524, and #487 needed a bump to 25.30. This gives back
2.44 KB.

## Consequences

**Advertised size drops `~25 kB` → `~23 kB`** (`Math.round(22.66)`); `import { stitch }`
...
| ------------------- | ---------- | ----------------------------- |
| whole entry         | 25.10 KB   | **22.90 KB** (~0.24 headroom) |
| `import { stitch }` | 20.25 KB   | 20.25 KB (unchanged)          |
| `stitchapi/auth`    | —          | **5.35 KB** (~0.24 headroom)  |
...
2. **`auth` as an atomic `extends` slot** + regression test. Small, independent,
   shippable on its own — and a bug fix, so it should not wait behind the move.
3. **The subpath move.** tsup entry, exports map, `index.ts` removal, repo-wide
   import rewrite, budgets down, advertised sizes, yakir re-baseline,
   migration-notes entry.
4. **Codegen + docs.** `from-curl` and `deriveAuth` emit the two-import form; the
   auth guides and reference switch their fences.
5. **Close out.** Mark ADR 0020 Superseded; close #487 with a pointer here.
...
- **Move only the five factories; keep `env`/`secretsFile`/`secretFrom` on the root.**
  Costs 0.27 KB gzip (22.93 vs 22.66) and puts two imports from two modules on every
