...
`StitchTraceEntry.stream?`. _Deliverable:_ a `contracts/` module that every other task
imports. _Accept:_ `tsc` passes; no behavioural code; existing `StitchPlayground`
...
"the only `fetch` is the sim shim," isolate caps for SR1. _Deliverable:_ a checklist
in this folder that T-α turns into tests. _Accept:_ every SANDBOX §7 + §9 line maps to
...
handler registry typed against the C1 contract, build wiring. _Accept:_ `pnpm -r build`
...
`fetch` is taken from injected scope. _Accept:_ a Tier-1 snippet runs unmodified in a
...
_Accept:_ deterministic; unit tests per behaviour.

**S3 — Streaming/SSE + LLM endpoint (T2).** Chunked + SSE token streaming and a
chat/completions-shaped endpoint with tool-call JSON. _Accept:_ emits an
...
_Accept:_ drift response fails a sample Zod schema; flaky succeeds on the Nth try
deterministically.
...
_Accept:_ identical handler → identical response in jsdom and node; unknown host 404s
...
_Accept:_ TS annotations erased; a syntax error yields a transpile-phase `RunError`, no throw.
...
`worker.terminate()`, `signal` wiring, transpile via R2. _Accept:_ SANDBOX §9 browser
...
self-describing endpoint listing available demo routes. _Accept:_ `GET /__sandbox`
...
composing browser (+ later server) runners. _Accept:_ each core export routes to the
...
Mermaid build-stitch DAG. _Accept:_ a streaming snippet renders progressively; a
...
_Accept:_ all SANDBOX §9 boxes are green in CI.

### Wave 6 — sign-off (T3)

**I1 — Integration & acceptance.** Run the full matrix end-to-end in the docs app,
confirm the §9 criteria, file follow-ups for Phase 3 (SR1). _Accept:_ SANDBOX §9 met;
SR1 scoped.
