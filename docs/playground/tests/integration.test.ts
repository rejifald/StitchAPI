...
import {
    allHandlers,
    registerAllHandlers,
} from '../../../packages/sandbox-sim/src/handlers/index';
...
async function main(): Promise<void> {
    ...
    await check('/status/500 → 500', async () => {
        ...
        assert(
            body.status === 500,
            `Expected body.status === 500, got ${String(body.status)}`,
        );
    });
    ...
    await check('/malformed → 200 with HTML (non-JSON) body', async () => {
        ...
        assert(
            ...
            'Expected HTML body to start with <!DOCTYPE html>',
        );
    ...
    await check(
        ...
        async () => {
            ...
            assert(
                ...
                `DRIFT: body.id should be string, got ${typeof body.id}`,
            );
    ...
    await check(
        ...
        async () => {
            ...
            assert(
                ...
                `body.id should be number, got ${typeof body.id}`,
            );
            assert(
                typeof body.name === 'string',
                `body.name should be string, got ${typeof body.name}`,
            );
    ...
    await check('/limited → 429 + Retry-After header', async () => {
        ...
        assert(
            retryAfter === '1',
            `Expected Retry-After: 1, got ${retryAfter}`,
        );
        const body = (await res.json()) as Record<string, unknown>;
        assert(
            body.error === 'rate_limited',
            `Expected error=rate_limited, got ${String(body.error)}`,
        );
        assert(
            body.retryAfterSeconds === 1,
            `Expected retryAfterSeconds=1, got ${String(body.retryAfterSeconds)}`,
        );
    });
    ...
    await check('/auth/me → 401 without bearer (§9 auth)', async () => {
        ...
        assert(
            ...
            `Expected error=unauthorized, got ${String(body.error)}`,
        );
    ...
    await check('/auth/me → 200 with bearer token (§9 auth)', async () => {
        ...
        assert(
            ...
            'Expected body.username to be a string',
        );
        assert(
            body.credentialKind === 'bearer',
            `Expected credentialKind=bearer, got ${String(body.credentialKind)}`,
        );
    });
    ...
    await check('GET /stream → yields multiple chunks', async () => {
        ...
        assert(
            ...
            'Expected a ReadableStream body',
        );
        const text = await res.text();
        assert(
            text.includes('chunk-alpha'),
            'Expected stream to contain chunk-alpha',
        );
        assert(
            text.includes('chunk-delta'),
            'Expected stream to contain chunk-delta',
        );
    });
    ...
    await check(
        ...
        async () => {
            ...
            assert(
                text.includes('[DONE]'),
                'Expected SSE stream to end with [DONE]',
            );
            assert(
                text.includes('Hello'),
                'Expected SSE stream to contain "Hello" token',
            );
            assert(
                text.includes('"finish_reason":"stop"'),
                'Expected SSE stream to have finish_reason:stop',
            );
    ...
    await check(
        ...
        async () => {
            ...
            assert(
                text.includes('[DONE]'),
                'Expected tool SSE stream to end with [DONE]',
            );
            assert(
                text.includes('tool_calls'),
                'Expected SSE stream to contain tool_calls',
            );
            assert(
                text.includes('get_weather'),
                'Expected SSE stream to contain tool name',
            );
            assert(
                text.includes('"finish_reason":"tool_calls"'),
                'Expected tool_calls finish_reason',
            );
        },
    );
    ...
    await check(
        'Unknown host → sandbox-404, no real network (SEC-03)',
        async () => {
            const res = await simFetch('https://example.com/x');
            assert(res.status === 404, `Expected 404, got ${res.status}`);
            const body = (await res.json()) as Record<string, unknown>;
            assert(
                body.error === 'sandbox_not_found',
                `Expected sandbox_not_found, got ${String(body.error)}`,
            );
            assert(
                typeof body.message === 'string' &&
                    (body.message as string).includes(
    ...
    await check(
        'Determinism: two sandbox-404 responses → identical bodies (SEC-05)',
        async () => {
    ...
    await check(
        ...
        () => {
            ...
            assert(
                scan.ambiguous === true,
                `Expected ambiguous:true, got ${String(scan.ambiguous)}`,
            );
            assert(
                scan.tier === 'browser',
                `Expected tier:browser, got ${scan.tier}`,
            );
            // ambiguous must NEVER route to server — the only safe default is browser
            assert(
                scan.tier !== 'server',
                'Ambiguous access must not route to server',
            );
        },
    );
    ...
    await check(
        'scanSurface: malformed input → fail-safe browser, no throw (SEC-48)',
        () => {
            let threw = false;
    ...
    await check('GET /__sandbox → 200 + routes catalogue', async () => {
        ...
        assert(
            ...
            'Expected routes array in catalogue',
        );
    ...
    await check(
        ...
        async () => {
            ...
            assert(
                ...
                `Expected object=chat.completion, got ${String(body.object)}`,
            );
    ...
    await check(
        ...
        async () => {
            ...
            assert(
                body.data.length === 3,
                `Expected 3 users, got ${body.data.length}`,
            );
            assert(body.total === 3, `Expected total=3, got ${body.total}`);
            const first = body.data[0] as Record<string, unknown>;
            assert(
                first.id === 1,
                `Expected first user id=1, got ${String(first.id)}`,
            );
            assert(
                first.name === 'Alice Liddell',
                `Expected Alice Liddell, got ${String(first.name)}`,
            );
        },
    );
    ...
    await check('GET /users/2 → Bob Hoskins (§9 basic fetch)', async () => {
        ...
        assert(
            body.data.id === 2,
            `Expected id=2, got ${String(body.data.id)}`,
        );
        assert(
            body.data.name === 'Bob Hoskins',
            `Expected Bob Hoskins, got ${String(body.data.name)}`,
        );
    });
