...
| No `globalThis` state bleed between two consecutive runs | **proven-by-R1-harness** — `browser-runner.test.ts` SEC-36/37 (fresh thread per run; globalThis.__bleed not present in run 2) | Browser Worker isolation (real CSP + Worker scope) is **browser-deferred** |
...
| **SEC-02** Known demo host → sim handler, 0 real sockets | **covered-in-node** — `integration.test.ts` checks #21/22 (demo.stitchapi.dev responses come from shim); `dispatch.test.ts` T7 | Real-socket spy is **browser-deferred** |
